...
|-----------|-------|------------|
| test_wellness_buddy.py | 26 | Core module workflow (emotion, patterns, alerts, profile) |
...
Following the core implementation, the system was extended with a modular research-grade architecture that separates concerns into explicit agent stages, adds transformer-backed emotion inference, introduces neural forecasting, and provides scientific evaluation utilities. These extensions are documented in `RESEARCH_REPRODUCIBILITY_GUIDE.md`.

### 4.8.1 Modular Agent Pipeline (`agent_pipeline.py`)

The processing chain is decomposed into five single-responsibility agents:

```
User Message
   |
   v
[EmotionAnalysisAgent]   — classify_emotion_ml()
   |
   v
[PatternTrackingAgent]   — sliding window + volatility + recovery + drift
   |
   v
[ForecastingAgent]       — OLS / EWMA / SimpleGRU comparison
   |
   v
[AlertDecisionAgent]     — contextual crisis + sustained distress
   |
   v
[ResponseGenerationAgent]
```

The orchestrator `WellnessAgentPipeline.process_turn()` calls each agent in sequence and returns a structured result dict containing `emotion`, `patterns`, `forecasting`, `alert`, and `response`. This replaces the monolithic `WellnessBuddy` call path for research mode and enables isolated unit testing of each stage.

### 4.8.2 Authentication Manager (`auth_manager.py`)

A dedicated `AuthManager` class provides bcrypt-based password management separate from `UserProfile`:

- `hash_password(password)`: bcrypt with auto-generated salt (work factor 12); raises `ValueError` for passwords below `config.MIN_PASSWORD_LENGTH`
- `verify_password(password, hash)`: constant-time bcrypt comparison; returns `bool`
- `is_locked_out(failed_attempts)`: per-session lockout at 5 failed attempts (vs. account lockout of 3 in `UserProfile`)
- `validate_password_strength(password)`: returns `(ok: bool, message: str)` for UI feedback

`UserProfile.verify_password()` migrates legacy SHA-256 hashes to bcrypt transparently on first correct login.

### 4.8.3 Transformer-Based Emotion Inference (`models/emotion_transformer.py`)

`EmotionTransformer` provides a lazy-loading adapter for DistilRoBERTa fine-tuned on GoEmotions:

- Falls back to `classify_keywords_only()` when the `transformers` library is absent (guaranteeing operation in constrained environments)
- Exposes `emotion_probabilities` (dict mapping emotion class → normalised confidence) for calibrated multi-class output
- `ContextualCrisisAdapter` adds `crisis_probability` and `suicidal_ideation_probability` to the result dict for nuanced risk scoring without binary keywords

`EmotionAnalyzer.classify_emotion_ml()` integrates both adapters, attaching an `xai_explanation` field generated by `explainability.py`.

### 4.8.4 Neural Forecasting Baseline (`prediction_agent.py` — `SimpleGRUForecaster`)

`SimpleGRUForecaster` provides a lightweight GRU-style sequence model that runs in pure Python (no PyTorch/TensorFlow required):

```python
class SimpleGRUForecaster:
    def __init__(self, lookback=4, hidden_decay=0.65, learning_rate=0.03, epochs=180):
        ...
    def fit(self, history):    # SGD on windowed sequences
    def predict(self, history): # next-step sentiment forecast
```

`compare_models(history, research_mode=True)` returns MAE/RMSE for OLS, EWMA, and SimpleGRU over a leave-one-out protocol, enabling the three-way ablation reported in Appendix C.

### 4.8.5 Scientific Evaluation Utilities (`evaluation_framework.py`, `research_evaluation.py`)

**`evaluation_framework.py`** provides five canonical distress scenario generators:
- `generate_gradual_decline_scenario()`: linear decline from +0.4 to −0.6 over *n* steps
- `generate_sudden_drop_scenario()`: stable baseline with acute event at configurable index
- `generate_recovery_scenario()`: low baseline recovering to positive over *n* steps
- `generate_stable_positive_scenario()`: control condition
- `generate_mixed_volatile_scenario()`: high-variance alternating trajectory

Statistical helpers: Welch's two-sample t-test, Pearson correlation with *p*-value, 95% confidence intervals, and detection confusion-matrix metrics (sensitivity/specificity).

**`research_evaluation.py`** provides dataset evaluation utilities compatible with GoEmotions, EmotionLines, and DailyDialog:
- `load_emotion_dataset(path)`: supports `.jsonl`, `.json`, `.csv` with automatic label normalisation
- `evaluate_classifier(samples, classifier_fn)`: returns precision/recall/macro-F1/micro-F1/confusion matrix
- `format_summary_table(report)`: produces IEEE-style formatted output for tables

### 4.8.6 Optional API Service and Docker (`api_service.py`, `Dockerfile`)

`api_service.py` wraps the pipeline in a FastAPI service for mobile/cloud integration:
...
- `POST /v1/chat` — per-user stateless turn via `WellnessAgentPipeline`
- `GET /v1/model-info` — reports transformer/crisis adapter availability and forecasting models

A `Dockerfile` enables containerised deployment:

```bash
docker build -t ai-wellness-buddy .
docker run -p 8000:8000 ai-wellness-buddy
```

The API is intentionally optional: all core functionality (CLI, Streamlit UI) remains operable without FastAPI or Docker.

### 4.8.7 Modular UI Package (`ui/`)

The Streamlit UI was refactored into a `ui/` Python package with four sub-modules:

| Sub-module | Responsibility |
|------------|---------------|
| `ui/theme.py` | CSS generation (`get_theme_css`): dark mode, three theme variants (calm/clinical/modern), risk-level colour variables |
| `ui/charts.py` | 11 Plotly chart factories: sentiment timeline, emotion donut, risk gauge, weekly chart, sparkline, moving-average, risk history, emotion heatmap, emotion journey line, stress intensity gauge, emotion probability bar |
| `ui/layout.py` | 13 HTML component builders: hero section, chat header, user/risk badges, session info, streak card, waveform, wellness sidebar, emotional avatar, wellness illustration |
| `ui/animations.py` | Canvas particle background, breathing circle, guided-breathing message, ambient soundscapes (4 types) |

### 4.8.8 Bilingual Tamil/English and Voice Support (`language_handler.py`, `voice_handler.py`)

`language_handler.py` provides:
- Tanglish (Tamil in Roman script) auto-detection via keyword matching across 6 emotion classes
- Tamil Unicode response pools for each emotion class
- Bilingual (Tamil + English mixed) response pool
- `LanguageHandler.get_response(emotion, mode)`: returns appropriate-language response string

`voice_handler.py` provides:
- `VoiceHandler.text_to_speech(text, lang)`: gTTS synthesis with `ta`/`en` language codes
- `VoiceHandler.speech_to_text()`: Google Speech Recognition (`en-IN` / `ta-IN` locales)
- In-browser audio recording via Streamlit's `st.audio_input()`

Language preference is stored in `UserProfile.profile_data['language_preference']` and applied to every session automatically.

### 4.8.9 Explainability Layer (`explainability.py`)

`explainability.py` generates human-readable XAI explanations for every emotion classification:

```python
from explainability import generate_explanation
explanation = generate_explanation(text, emotion_data)
# Returns: {
#   'primary_emotion': 'anxiety',
#   'confidence': 0.72,
#   'key_indicators': ['stressed', 'overwhelmed', 'can\'t cope'],
#   'sentiment_contribution': 'negative polarity reinforced distress signal',
#   'model_source': 'keyword_hybrid'
# }
```

The Streamlit Chat tab displays this explanation in a collapsible "Emotion Analysis" expander beneath each response, showing the `key_indicators` as a bulleted list.
...
This chapter presented the complete implementation of AI Wellness Buddy, including both the core system and its research-grade extensions:

1. **Development Environment**: Python 3.12, cross-platform, minimal hardware requirements
2. **Technology Stack**: TextBlob for local NLP, Fernet encryption, Streamlit web framework — all operating without external API calls; optional FastAPI/Docker for scalable deployment
3. **EmotionAnalyzer**: Three-signal fusion (statistical + distress keywords + abuse indicators) with transformer adapter (`EmotionTransformer`/`ContextualCrisisAdapter`) providing calibrated probability outputs and XAI explanations
4. **PatternTracker**: Sliding-window deque with consecutive-distress counter, volatility/recovery/persistence/drift metrics, O(1) per-message updates
...
6. **DataStore**: Fernet-encrypted JSON persistence, file permission restrictions (0o600), backup and integrity-hash support
7. **UserProfile**: bcrypt password hashing (via `AuthManager`), SHA-256 legacy auto-migration, account lockout, session timeout, 7 demographic fields, 365-day emotional history
8. **Forecasting**: OLS + EWMA + `SimpleGRUForecaster` (pure-Python neural baseline) with leave-one-out MAE/RMSE comparison via `compare_models()`
9. **Agent Pipeline**: `WellnessAgentPipeline` orchestrates 5 single-responsibility agents (EmotionAnalysis / PatternTracking / Forecasting / AlertDecision / ResponseGeneration)
10. **Scientific Evaluation**: `evaluation_framework.py` (5 scenario generators, statistical tests) + `research_evaluation.py` (GoEmotions/EmotionLines/DailyDialog dataset loaders, precision/recall/F1)
